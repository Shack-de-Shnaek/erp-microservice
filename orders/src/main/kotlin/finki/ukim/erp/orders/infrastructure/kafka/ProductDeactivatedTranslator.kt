 ...
 * service's.
 *
 * On the way in it is a [ProductDeactivatedExternalEventDTO], a mirror of somebody else's JSON. On
 ...
 * domain value, because a deactivated product does not name the thing to change - it takes a query
 ...
 * translation. The lookup lives in [finki.ukim.erp.orders.handlers.ProductDeactivatedEventHandler]
 * instead, and this class stays what it claims to be: types in, types out, no dependencies.
 */
@Service
class ProductDeactivatedTranslator {

    fun toDeactivatedProduct(event: ProductDeactivatedExternalEventDTO): ProductId =
